...
M3 provides two scratch filesystems — `mh42_scratch` and `mh42_scratch2`. **Use `mh42_scratch2` for all new data** — it follows the convention consistently. `mh42_scratch` has accumulated clutter from earlier usage.

Follow the **same folder structure** as `mh42/`:

```bash
mkdir -p /home/${USER}/mh42_scratch2/{your_project_folder}/stack_{your_name}
```

!!! warning
    Scratch is flushed periodically. Do not rely on it for anything you need long-term — archive to [S Drive](../storage/s-drive.md) or move to `mh42/` before the purge.
